...
function playHero(){
  ...
  ln(500,'<span class="rm">remote:</span> ref accepted · run <span class="c-cyn">8f3a2c</span> queued (repo/feat/rotate-keys)');
...
var STEPS=[
 {n:'receive · git ingress',d:'<b>Authenticate the uplink, attribute the actor, resolve the repo.</b> Only the Git smart protocol is accepted — no shells, no forwarding. Unknown repo? Oberth fetches it from your upstream and caches it. Upstream down? The cached clone is served with a stale warning instead of failing.',
...
var SETUP_COMBOS={
 'mac|kind':{
  ...
  tldr:['brew install oberthci/tap/oberth','oberth install --install-secretstore-dev','kubectl exec -n oberth deploy/oberth -- oberth upstream add codeberg ssh://git@codeberg.org/your-org','kubectl exec -n oberth deploy/oberth -- oberth uplink add "$(cat ~/.ssh/id_ed25519.pub)" agent@runner']
 },
 'linux|k3s':{
