...
var ORB='<svg class="orb" viewBox="0 0 64 40" aria-hidden="true">'
 ...
 +'</svg>';
var FEATURES=[
 {t:'Git ingress & clone-through cache',one:'SSH in, smart protocol only. Clone paths match your upstream: <code>codeberg/your-org/your-repo</code>. Bare names and org/repo still work.',
  b:'SSH on a fixed NodePort, <code>git-upload-pack</code>/<code>git-receive-pack</code> and nothing else — no shells, no PTYs, no forwarding. Repositories are addressed by their full upstream identity: <code>upstream/org/repo</code> — clone <code>codeberg/acme/payments-api</code> and the path matches the repo\'s Codeberg home. Bare names and <code>org/repo</code> shorthands still resolve when unambiguous. Repos are fetched from your upstream on demand, cached, and stale-served through outages. Force-pushed feature branches are accepted — agents rebase constantly. Tags are creation-only: deletion, movement and collisions are rejected at the door.',
  c:['<b>NodePort</b> 30022','<b>paths</b> upstream/org/repo','<b>cache</b> clone-through + stale-serve','<b>tags</b> creation-only']},
 ...
 {t:'Argo Workflows — one pod per step',one:'Each push submits an Argo Workflow. One pod per DAG task, FIFO queue, supersession, bounded resources.',
  b:'Every push submits an Argo Workflow — one pod per DAG task, each with its own container image, exit code, and named log slice. Runs queue FIFO with a concurrency knob (default 3 on a single node). A newer push to the same branch <b>supersedes</b> the in-flight run, so agents never queue behind their own dead commits. Workflows carry resource requests and limits per template, an active deadline, and TTL cleanup — one unbounded job can\u2019t evict the CI system that spawned it.',
  c:['<b>queue</b> FIFO · default 3','<b>supersede</b> new push cancels stale run','<b>identity</b> no SA token · non-root 65534','<b>isolation</b> one pod per step']},
 {t:'Right-sized containers',one:'Pipeline steps run in right-sized containers — golang for Go builds, hashicorp/terraform for IaC, trivy for scanning.',
  b:'Each step runs in its own container — golang for Go builds, hashicorp/terraform for IaC, trivy for scanning. Images are digest-pinned in the workflow YAML. No ambient toolchain, no shared PATH.',
  c:['<b>images</b> per-step, declared in YAML','<b>tools</b> pinned with sha256','<b>caches</b> shared volumes, per-repo']},
 {t:'Argo Workflow pipeline',one:'Your pipeline is Argo Workflow YAML — standard, auditable, explicit. Oberth handles submission and identity.',
  b:'<code>.oberth/build.yaml</code> for branches, <code>.oberth/release.yaml</code> for tags. Standard Argo Workflow resources with DAG tasks and container templates. Oberth submits the YAML, sets metadata, forces the ServiceAccount identity by trigger tier, and injects the source checkout. Each template runs in its own container with its own exit code and named log slice. The server never executes repository code.',
  c:['<b>timeout</b> per-template deadline','<b>workflow deadline</b> configurable','<b>logs</b> per-template named slices']},
 {t:'Issues as work queue',one:'Red CI files exactly one issue per repo+branch. Green auto-closes it. Locks stop agent pile-ups.',
  ...
  c:['<b>dedupe</b> 1 open issue / repo+branch','<b>lock</b> 5-min TTL · uplink-owned','<b>scope</b> sqlite = CI queue · upstream = tracker of record']},
 {t:'Promote',one:'Green-gate the exact SHA, prove the merged tree, push non-forced. Append-only, no retries in place.',
  b:'Promotion is one definition, applied once: verify CI is really green for the exact SHA, fetch the target, do a standard local merge, run CI on the <b>merged tree</b>, and push non-forced. If the target moved meanwhile, the non-fast-forward rejection is the free concurrency guard — that promotion is terminal, fixes go to the feature branch, new SHA, new promote. If the merge is a fast-forward, the tested tree is already exactly that tree: no wasteful re-run.',
  c:['<b>records</b> append-only','<b>push</b> never forced','<b>fast-forward</b> skips the re-run']},
 {t:'Release runs',one:'Tags release only from promoted history. Secrets: declared, approved, fetched in-pod via envconsul, memory-only.',
  b:'A tag gets a release run only if its commit is reachable from the default branch — one <code>merge-base --is-ancestor</code> check, so nobody releases an arbitrary un-promoted SHA by slapping a tag on it. The declared secret paths (parsed statically, never executed) are checked against the administrator approval table at admission. Release steps wrap their command with <code>envconsul</code>, which authenticates to OpenBao via Kubernetes auth using the credentialed ServiceAccount\u2019s projected token and injects the declared secrets into the child process environment. The Oberth server never sees the secret values. Branch pushes get only approved upstream-scoped secrets. Release credentials are never reachable from a branch pipeline.',
  c:['<b>gate</b> ancestry from default branch','<b>delivery</b> in-pod envconsul · Kubernetes auth','<b>redaction</b> repository-invoked helper, not automatic']},
 ...
 {t:'Secret store release secrets',one:'Declared in the workflow annotation. Credentialed steps fetch secrets in-pod via envconsul and Kubernetes auth — the server never sees the values.',
  b:'Point Oberth at your OpenBao or HashiCorp Vault. Release steps wrap their command with <code>envconsul</code>, which authenticates via Kubernetes auth using the credentialed ServiceAccount\u2019s projected token and injects the declared secrets into the child process environment. The Oberth server never sees the secret values and has no code path that accepts a store admin token. A missing secret, an unreachable store, or a path outside the administrator approval table fails the run at admission, with the exact path in the error. Log redaction is a repository-invoked helper (used in Oberth\u2019s own release pipeline), not an admission-enforced boundary. Setup is one drift-guarded script \u2014 <code>curl -fsSL https://oberth.ci/setup-secretstore.sh | bash</code> \u2014 run next to your store; <code>oberth secretstore verify</code> proves the whole trust chain from inside the pod, no credentials needed.',
  c:['<b>store</b> OpenBao · HashiCorp Vault','<b>tokens</b> read-only · 10-min TTL · revoked after fetch','<b>delivery</b> in-pod envconsul · Kubernetes auth','<b>branch builds</b> zero credentials']}
];
...
(function(){
  ...
  var last=0,rafHandle=0,running=false;
  function loop(ts){
    if(!last)last=ts;
    var dt=Math.min(0.033,(ts-last)/1000);last=ts;
    if(W&&H){ step(dt*TS); draw(); }
  ...
  function stopLoop(){
    if(!running)return;
    running=false;
    cancelAnimationFrame(rafHandle);rafHandle=0;last=0;
  }
  /* start/stop when the home page is shown/hidden */
  window.obSimStart=startLoop;
  window.obSimStop=stopLoop;
  ...
  setStatus();
  if(reduce&&W&&H){
    /* reduced-motion: advance to the static state, draw once, schedule nothing */
  ...
  cv.addEventListener('click',function(){for(var k=0;k<110;k++)step(1/50*TS);if(!running&&!reduce){draw()}});
})();
...
var ROUTES=['home','why','pipeline','agents','trust','setup','alpha','try'];
var TITLES={
  home:'Oberth — Put your CI where your agent is',
  why:'Oberth — no access change slips through a git commit',
  pipeline:'Oberth — Argo Workflow pipeline',
  agents:'Oberth — eighteen MCP tools for AI agents',
  trust:'Oberth — the trigger is the security boundary',
  setup:'Oberth — get started',
  alpha:'Oberth — alpha pilot ledger',
  try:'Oberth — get started'
};
function route(){
  var h=(location.hash||'#/').replace('#/','')||'home';
  ...
  scrollTo(0,0);
  if(h==='home'){
    if(window.obSimResize)setTimeout(window.obSimResize,30);
    if(window.obSimStart)window.obSimStart();
  }else{
    if(window.obSimStop)window.obSimStop();
  }
  ...
  if(h==='setup')setTimeout(bindSetupWizard,0);
}
...
function playHero(){
  ...
  var t=0,pEl=null;
  function ln(delay,html,cls){
    t+=delay;
    heroTimer.push(setTimeout(function(){
      if(pEl){var cr=pEl.querySelector('.caret');if(cr)cr.parentNode.removeChild(cr);pEl=null}
  ...
  function pr(delay){
    t+=delay;
    heroTimer.push(setTimeout(function(){
      pEl=document.createElement('div');pEl.className='tl show';
      pEl.innerHTML='<span class="pr">$</span> <span class="caret"></span>';
      heroLog.appendChild(pEl);
    },t));
  }
  /* part 1 — git push returns after three lines */
  ln(200,'<span class="pr">$</span> git push oberth feat/rotate-keys');
  ln(700,'<span class="rm">remote:</span> [oberth] uplink <span class="c-pur">agent-07@runner</span>');
  ln(500,'<span class="rm">remote:</span> ref accepted · run <span class="c-cyn">8f3a2c</span> queued (<span class="c-f">codeberg/acme/payments-api</span>)');
  ln(500,'<span class="rm">remote:</span> watch: https://watch.oberth.ci');
  pr(500);
  /* part 2 — async results, queried separately */
  ln(1400,'<span class="cm"># server-side (visible via MCP or dashboard):</span>','hero-gap');
  ln(600,'<span class="pr">$</span> oberth status feat/rotate-keys');
  ln(600,'setup ✓ &nbsp;lint ✓ &nbsp;test ✓ &nbsp;build ✓<span class="st grn">GREEN</span>');
  ln(500,'branch synced → upstream<span class="st syn">SYNCED</span>');
  ln(500,'issue #47 auto-closed<span class="st dim">CLOSED</span>');
  pr(700);
}
...
var STEPS=[
 {n:'receive · git ingress',d:'<b>Authenticate the uplink, attribute the actor, resolve the repo.</b> Only the Git smart protocol is accepted — no shells, no forwarding. Clone paths match your upstream home: <code>codeberg/acme/payments-api</code>. Bare names and org/repo still work as shorthands. Unknown repo? Oberth fetches it from your upstream and caches it.',
  t:'<div class="tl show"><span class="pr">$</span> git clone ssh://git@oberth:30022/<span class="c-f">codeberg/acme/payments-api</span>.git</div><div class="tl show"><span class="rm">remote:</span> uplink <span class="c-pur">agent-07@runner</span> · repo cached ✓</div><div class="tl show"><span class="cm"># bare names and org/repo shorthands also work</span></div><div class="tl show"><span class="pr">$</span> git push oberth feat/rotate-keys</div><div class="tl show"><span class="rm">remote:</span> force-update on feature branch <span class="st grn ml0">ACCEPTED</span></div>'},
 {n:'run · argo workflow',d:'<b>One push, one Argo Workflow.</b> Oberth submits the repository\'s <b>build.yaml</b> as an Argo Workflow; each DAG task runs in its own container with its own exit code and named logs. FIFO queue, default 3 concurrent; a newer push to the same branch supersedes the stale run.',
 ...
 {n:'record · red or green',d:'<b>Red creates exactly one issue per repo+branch</b> — repeat reds update it, so an agent iterating on a failure can\u2019t spam the queue. Green force-syncs the exact tested branch to your repo and auto-closes the issue. Green work always exists upstream — no local-only limbo.',
 ...
 {n:'release · tag run',d:'<b>Tag the promoted commit and push the tag.</b> Oberth checks it\u2019s reachable from the default branch, runs the release workflow with exactly the declared and approved secrets — fetched in-pod from OpenBao via envconsul and Kubernetes auth — tests against the real published artifacts, and syncs the tag upstream.',
  t:'<div class="tl show"><span class="pr">$</span> git push oberth v0.13.44</div><div class="tl show"><span class="rm">gate:</span> merge-base --is-ancestor main ✓</div><div class="tl show"><span class="rm">run:</span> release · secrets: <span class="c-yel">r2-publisher, cosign-key</span> <span class="cm">· in-pod envconsul · Kubernetes auth</span></div><div class="tl show"><span class="rm">verify:</span> published artifacts re-downloaded, checksums ✓<span class="st grn">GREEN</span></div><div class="tl show"><span class="rm">tag:</span> synced → upstream<span class="st syn">COMPLETE</span></div>'}
];
...
var K8S={
 k3s:{t:'k3s <em>single binary, systemd-managed</em>',c:'<span class="c-cm"># Download k3s installer, inspect, then run</span>\ncurl -sfL https://get.k3s.io -o k3s-install.sh\nless k3s-install.sh\nsh k3s-install.sh --disable=traefik --write-kubeconfig-mode 644'},
 ...
 k0s:{t:'k0s <em>single binary, controller + worker</em>',c:'<span class="c-cm"># Download k0s installer, inspect, then run</span>\ncurl -sSfL https://get.k0s.sh -o k0s-install.sh\nless k0s-install.sh\nsudo sh k0s-install.sh\nsudo k0s install controller --single --enable-worker\nsudo k0s start'}
};
...
/* ---------- setup page: platform + k8s flavor wizard ---------- */
var SETUP_PLATFORMS={
 mac:{label:'macOS',sub:'Apple Silicon · M1 / M2 / M3 / M4 / M5',k8s:['kind']},
 linux:{label:'Linux',sub:'x86_64 · bare metal or VM',k8s:['k3s','k0s']}
};
var SETUP_K8S_META={
 kind:{label:'kind',sub:'in Docker · needs OrbStack or Docker Desktop',rec:false},
 k3s:{label:'k3s',sub:'single binary, systemd-managed',rec:true},
 k0s:{label:'k0s',sub:'zero-friction, single binary',rec:false}
};
var SETUP_COMBOS={
 'mac|kind':{
  title:'macOS Apple Silicon + kind',
  blocks:[
   {k:'prereq',h:'prerequisites',c:'<span class="c-cm"># OrbStack (recommended) or Docker Desktop</span>\n<span class="c-cm"># If neither is installed, kind won’t work — that’s on you, go install one first</span>'},
   {k:'cli',h:'install the Oberth CLI',c:'<span class="c-cm"># Install Oberth CLI</span>\nbrew install oberthci/tap/oberth'},
   {k:'hero',h:'create kind cluster + install everything <span class="info-tip" data-tip="Dev mode: in-memory OpenBao, auto-unsealed, root token \'root\'. For evaluation only — not production.">ⓘ</span>',c:'<span class="c-cm"># Create kind cluster + install everything</span>\noberth install <span class="c-f">--install-secretstore-dev</span>'},
   {k:'bootstrap',h:'bootstrap identity',c:'<span class="c-cm"># Done! Bootstrap identity:</span>\nkubectl exec -n oberth deploy/oberth -- oberth <span class="c-f">upstream add</span> codeberg ssh://git@codeberg.org/your-org  <span class="c-cm"># <span class="info-tip" data-tip="Mints a deploy key — register it at your Git host. Oberth fetches repos on demand.">ⓘ</span></span>\nkubectl exec -n oberth deploy/oberth -- oberth <span class="c-f">uplink add</span> \"$(cat ~/.ssh/id_ed25519.pub)\" <span class="c-y">agent@runner</span>  <span class="c-cm"># <span class="info-tip" data-tip="Registers your SSH key. The bearer token is printed once — save it for MCP/API access.">ⓘ</span></span>'},
   {k:'clone',h:'clone your first repo <span class="info-tip" data-tip="Paths match your upstream: upstream/org/repo. Bare names and org/repo shorthands also work.">\u24d8</span>',c:'<span class="c-cm"># Clone a repo \u2014 path matches your upstream home</span>\ngit clone ssh://git@<span class="c-y">YOUR_HOST</span>:30022/<span class="c-f">codeberg/your-org/your-repo</span>.git\n<span class="c-cm"># Shorthand also works: git clone ssh://.../<span class="c-f">your-repo</span>.git</span>'}
  ],
  tldr:['brew install oberthci/tap/oberth','oberth install --install-secretstore-dev','kubectl exec -n oberth deploy/oberth -- oberth upstream add codeberg ssh://git@codeberg.org/your-org','kubectl exec -n oberth deploy/oberth -- oberth uplink add "$(cat ~/.ssh/id_ed25519.pub)" agent@runner','git clone ssh://git@YOUR_HOST:30022/codeberg/your-org/your-repo.git']
 },
 'linux|k3s':{
  title:'Linux x86_64 + k3s',
  blocks:[
   {k:'k8s',h:'install k3s',c:'<span class="c-cm"># Download k3s installer, inspect, then run</span>\ncurl -sfL https://get.k3s.io -o k3s-install.sh\nless k3s-install.sh\nsh k3s-install.sh <span class="c-f">--disable=traefik --write-kubeconfig-mode</span> 644'},
   {k:'cli',h:'install the Oberth CLI <span class="info-tip" data-tip="Auto-detects OS and architecture, downloads the latest version, verifies the SHA-256 checksum, and installs to /usr/local/bin. Pass --version to pin a specific release.">ⓘ</span>',c:'<span class="c-cm"># Install the Oberth CLI</span>\ncurl -fsSL https://oberth.ci/install.sh | sh'},
   {k:'hero',h:'install everything into the existing cluster <span class="info-tip" data-tip="Dev mode: in-memory OpenBao, auto-unsealed, root token \'root\'. For evaluation only — not production.">ⓘ</span>',c:'<span class="c-cm"># Install everything into existing cluster</span>\noberth install <span class="c-f">--install-secretstore-dev</span>'},
   {k:'bootstrap',h:'bootstrap identity',c:'<span class="c-cm"># Done! Bootstrap identity:</span>\nkubectl exec -n oberth deploy/oberth -- oberth <span class="c-f">upstream add</span> codeberg ssh://git@codeberg.org/your-org  <span class="c-cm"># <span class="info-tip" data-tip="Mints a deploy key — register it at your Git host. Oberth fetches repos on demand.">ⓘ</span></span>\nkubectl exec -n oberth deploy/oberth -- oberth <span class="c-f">uplink add</span> \"$(cat ~/.ssh/id_ed25519.pub)\" <span class="c-y">agent@runner</span>  <span class="c-cm"># <span class="info-tip" data-tip="Registers your SSH key. The bearer token is printed once — save it for MCP/API access.">ⓘ</span></span>'},
   {k:'clone',h:'clone your first repo <span class="info-tip" data-tip="Paths match your upstream: upstream/org/repo. Bare names and org/repo shorthands also work.">\u24d8</span>',c:'<span class="c-cm"># Clone a repo \u2014 path matches your upstream home</span>\ngit clone ssh://git@<span class="c-y">YOUR_HOST</span>:30022/<span class="c-f">codeberg/your-org/your-repo</span>.git\n<span class="c-cm"># Shorthand also works: git clone ssh://.../<span class="c-f">your-repo</span>.git</span>'}
  ],
  tldr:['curl -sfL https://get.k3s.io -o k3s-install.sh && sh k3s-install.sh --disable=traefik --write-kubeconfig-mode 644','curl -fsSL https://oberth.ci/install.sh | sh','oberth install --install-secretstore-dev','kubectl exec -n oberth deploy/oberth -- oberth upstream add codeberg ssh://git@codeberg.org/your-org','kubectl exec -n oberth deploy/oberth -- oberth uplink add "$(cat ~/.ssh/id_ed25519.pub)" agent@runner','git clone ssh://git@YOUR_HOST:30022/codeberg/your-org/your-repo.git']
 },
 'linux|k0s':{
  title:'Linux x86_64 + k0s',
  blocks:[
   {k:'k8s',h:'install k0s',c:'<span class="c-cm"># Download k0s installer, inspect, then run</span>\ncurl -sSfL https://get.k0s.sh -o k0s-install.sh\nless k0s-install.sh\nsudo sh k0s-install.sh\nsudo k0s install controller --single\nsudo k0s start\nsudo k0s kubeconfig admin &gt; ~/.kube/config'},
   {k:'cli',h:'install the Oberth CLI <span class="info-tip" data-tip="Auto-detects OS and architecture, downloads the latest version, verifies the SHA-256 checksum, and installs to /usr/local/bin. Pass --version to pin a specific release.">ⓘ</span>',c:'<span class="c-cm"># Install the Oberth CLI</span>\ncurl -fsSL https://oberth.ci/install.sh | sh'},
   {k:'hero',h:'install everything into the existing cluster <span class="info-tip" data-tip="Dev mode: in-memory OpenBao, auto-unsealed, root token \'root\'. For evaluation only — not production.">ⓘ</span>',c:'<span class="c-cm"># Install everything into existing cluster</span>\noberth install <span class="c-f">--install-secretstore-dev</span>'},
   {k:'bootstrap',h:'bootstrap identity',c:'<span class="c-cm"># Done! Bootstrap identity:</span>\nkubectl exec -n oberth deploy/oberth -- oberth <span class="c-f">upstream add</span> codeberg ssh://git@codeberg.org/your-org  <span class="c-cm"># <span class="info-tip" data-tip="Mints a deploy key — register it at your Git host. Oberth fetches repos on demand.">ⓘ</span></span>\nkubectl exec -n oberth deploy/oberth -- oberth <span class="c-f">uplink add</span> \"$(cat ~/.ssh/id_ed25519.pub)\" <span class="c-y">agent@runner</span>  <span class="c-cm"># <span class="info-tip" data-tip="Registers your SSH key. The bearer token is printed once — save it for MCP/API access.">ⓘ</span></span>'},
   {k:'clone',h:'clone your first repo <span class="info-tip" data-tip="Paths match your upstream: upstream/org/repo. Bare names and org/repo shorthands also work.">\u24d8</span>',c:'<span class="c-cm"># Clone a repo \u2014 path matches your upstream home</span>\ngit clone ssh://git@<span class="c-y">YOUR_HOST</span>:30022/<span class="c-f">codeberg/your-org/your-repo</span>.git\n<span class="c-cm"># Shorthand also works: git clone ssh://.../<span class="c-f">your-repo</span>.git</span>'}
  ],
  tldr:['curl -sSfL https://get.k0s.sh -o k0s-install.sh && sudo sh k0s-install.sh','sudo k0s install controller --single','sudo k0s start','sudo k0s kubeconfig admin > ~/.kube/config','curl -fsSL https://oberth.ci/install.sh | sh','oberth install --install-secretstore-dev','kubectl exec -n oberth deploy/oberth -- oberth upstream add codeberg ssh://git@codeberg.org/your-org','kubectl exec -n oberth deploy/oberth -- oberth uplink add "$(cat ~/.ssh/id_ed25519.pub)" agent@runner','git clone ssh://git@YOUR_HOST:30022/codeberg/your-org/your-repo.git']
 }
};
var setupCurPlatform='mac',setupCurK8s='kind';
function renderSetupK8sFlavors(){
  var grid=document.getElementById('k8sFlavorGrid');if(!grid)return;
  var plat=SETUP_PLATFORMS[setupCurPlatform];
  var html='';
  plat.k8s.forEach(function(k){
    var m=SETUP_K8S_META[k];
    html+='<button class="k8sflavor-card'+(k===setupCurK8s?' on':'')+'" data-k8sflavor="'+k+'" aria-pressed="'+(k===setupCurK8s?'true':'false')+'"><span class="kc-t">'+m.label+(m.rec?' <span class="qs-rec">recommended</span>':'')+'</span><span class="kc-s">'+m.sub+'</span></button>';
  });
  grid.innerHTML=html;
  grid.querySelectorAll('.k8sflavor-card').forEach(function(b){
    b.addEventListener('click',function(){
      setupCurK8s=b.dataset.k8sflavor;
      renderSetupK8sFlavors();
      renderSetupInstructions();
    });
  });
}
function renderSetupInstructions(){
  var combo=SETUP_COMBOS[setupCurPlatform+'|'+setupCurK8s];
  var step=document.getElementById('wizInstrStep'),body=document.getElementById('wizInstrBody'),label=document.getElementById('wizComboLabel');
  if(!combo||!step||!body)return;
  label.textContent=combo.title;
  var html='';
  combo.blocks.forEach(function(b){
    html+='<div class="code'+(b.k==='hero'?' wiz-hero':'')+'"><div class="code-h"><span>'+b.h+'</span><button class="cp">COPY</button></div><pre>'+b.c+'</pre></div>';
  });
  html+='<div class="code tldr"><div class="code-h"><span class="tldr-l">TLDR — paste and go</span><button class="cp">COPY ALL</button></div><pre>'+combo.tldr.join('\n')+'</pre></div>';
  body.innerHTML=html;
  step.classList.remove('show');
  requestAnimationFrame(function(){requestAnimationFrame(function(){step.classList.add('show')})});
}
function bindSetupWizard(){
  var root=document.getElementById('setupWizard');if(!root)return;
  if(root.dataset.bound)return;
  root.dataset.bound='1';
  root.querySelectorAll('.platform-card').forEach(function(b){
    b.addEventListener('click',function(){
      setupCurPlatform=b.dataset.platform;
      root.querySelectorAll('.platform-card').forEach(function(x){x.classList.remove('on');x.setAttribute('aria-pressed','false')});
      b.classList.add('on');b.setAttribute('aria-pressed','true');
      setupCurK8s=SETUP_PLATFORMS[setupCurPlatform].k8s[0];
      renderSetupK8sFlavors();
      renderSetupInstructions();
    });
  });
  root.addEventListener('click',function(e){
    var cp=e.target.closest?e.target.closest('.cp'):null;
    if(!cp)return;
    var pre=cp.closest('.code').querySelector('pre');
    if(navigator.clipboard)navigator.clipboard.writeText(pre.textContent);
    var o=cp.textContent;cp.textContent='COPIED ✓';setTimeout(function(){cp.textContent=o},1300);
  });
  renderSetupK8sFlavors();
  renderSetupInstructions();
}
