...
const (
	...
	maximumEnumerationCandidates = 2048
...
// RekorWitnessConfig selects the Rekor endpoint and its optional transport
// and log-identity trust anchors.
type RekorWitnessConfig struct {
	Endpoint  string
	TLSCACert *x509.CertPool
	PublicKey crypto.PublicKey
	// InsecureHTTP permits a plain-HTTP endpoint for DEVELOPMENT ONLY Rekor
	// deployments that do not expose a native TLS listener.
	InsecureHTTP bool
}

// NewRekorWitness creates a Rekor transparency log witness backed by the
// configured endpoint and derived from the persistent SSH host key material.
// When TLSCACert is non-nil it pins the HTTPS trust anchors, enabling
// self-hosted or air-gapped deployments behind a private CA. When PublicKey is
// non-nil it is the sole trusted Rekor log key; otherwise the public Sigstore
// TUF trust metadata is used.
func NewRekorWitness(ctx context.Context, hostKeyMaterial []byte, config RekorWitnessConfig) (*RekorWitness, error) {
	parsed, err := parseRekorEndpoint(config.Endpoint, config.InsecureHTTP)
	if err != nil {
		return nil, err
	}
	...
	var verifier rekorEntryVerifier
	if config.PublicKey != nil {
		verifier, err = newRekorPublicKeyVerifier(config.PublicKey)
		if err != nil {
			return nil, fmt.Errorf("audit anchor: load configured Rekor public key: %w", err)
		}
	} else {
...
func parseRekorEndpoint(endpoint string, insecureHTTP bool) (*url.URL, error) {
	parsed, err := url.ParseRequestURI(endpoint)
	validScheme := parsed != nil && (parsed.Scheme == "https" || insecureHTTP && parsed.Scheme == "http")
	if err != nil || strings.Contains(endpoint, "#") || parsed == nil || parsed.Host == "" || !validScheme || parsed.User != nil || parsed.Fragment != "" ||
		(parsed.Scheme == "https" && !strings.HasPrefix(endpoint, "https://")) ||
		(parsed.Scheme == "http" && !strings.HasPrefix(endpoint, "http://")) {
		requiredScheme := "HTTPS"
		if insecureHTTP {
			requiredScheme = "HTTP or HTTPS"
		}
		return nil, fmt.Errorf("audit anchor: Rekor endpoint must be an absolute %s URL without credentials or a fragment", requiredScheme)
	}
	return parsed, nil
}
...
func (witness *RekorWitness) AbandonedChainTip(ctx context.Context) (model.AuditWitness, bool, error) {
	...
	if len(candidates) > maximumEnumerationCandidates {
		return model.AuditWitness{}, false, fmt.Errorf(
			"audit anchor: Rekor identity search returned %d entries, exceeding the %d enumeration budget",
			len(candidates), maximumEnumerationCandidates,
		)
	}
	enumerationDeadline, cancelEnumeration := context.WithTimeout(ctx, enumerationTimeout)
	defer cancelEnumeration()
	var tip model.AuditWitness
	found := false
	ownCount := 0
	for _, uuid := range candidates {
		if enumerationDeadline.Err() != nil {
			return model.AuditWitness{}, false, fmt.Errorf(
				"audit anchor: Rekor identity enumeration timed out after verifying %d of %d candidates (%d own-chain)",
				ownCount, len(candidates), ownCount,
			)
		}
		actualUUID, entry, getErr := witness.api.Get(enumerationDeadline, uuid)
		if getErr != nil {
		...
		verified, verifyErr := witness.verifyEntry(enumerationDeadline, actualUUID, &entry)
		...
		ownCount++
		if ownCount > maximumIdentitySearchResults {
			return model.AuditWitness{}, false, fmt.Errorf(
				"audit anchor: Rekor identity contains %d own-chain entries, more than the supported %d",
				ownCount, maximumIdentitySearchResults,
			)
		}
...
func normalizedRekorUUID(uuid string) (string, error) {
	base, err := rekorUUIDFromIDString(uuid)
...
func (api *generatedRekorAPI) Get(ctx context.Context, uuid string) (string, models.LogEntryAnon, error) {
	...
	for actualUUID, entry := range response.Payload {
		requestedBase, requestedErr := rekorUUIDFromIDString(uuid)
		actualBase, actualErr := rekorUUIDFromIDString(actualUUID)
...
type rekorTrustedKeyVerifier struct {
	keys        map[string]signature.Verifier
	trustSource string
}

func newRekorPublicKeyVerifier(publicKey crypto.PublicKey) (*rekorTrustedKeyVerifier, error) {
	publicDER, err := x509.MarshalPKIXPublicKey(publicKey)
	if err != nil {
	...
	return &rekorTrustedKeyVerifier{
		keys:        map[string]signature.Verifier{hex.EncodeToString(logID[:]): verifier},
		trustSource: "configured Rekor public key",
	}, nil
}

func loadTUFRekorVerifier(ctx context.Context) (*rekorTrustedKeyVerifier, error) {
	...
	return &rekorTrustedKeyVerifier{keys: verifiers, trustSource: "TUF trust metadata"}, nil
}

func (verifier *rekorTrustedKeyVerifier) Verify(ctx context.Context, entry *models.LogEntryAnon) error {
	if entry == nil || entry.LogID == nil {
		return errors.New("rekor entry has no log ID")
	}
	key, ok := verifier.keys[strings.ToLower(*entry.LogID)]
	if !ok {
		return fmt.Errorf("rekor log ID %s is absent from %s", *entry.LogID, verifier.trustSource)
	}
