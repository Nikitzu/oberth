...
type AnchorStore interface {
	...
	VerifyAuditStateSince(context.Context, int64, []byte) (model.AuditHead, error)
	...
	VerifyAuditMutationStateUnanchored(context.Context, []model.AuditWitnessIntent, []model.AuditWitness, func(model.AuditHead, model.AuditAnchor) error) (model.AuditHead, model.AuditAnchor, error)
...
// ManagerConfig configures the audit integrity manager. Store is always
// required: the local gap-free SHA-256 hash chain is not optional. Authority
// (RFC 3161 timestamps) and Witness (Rekor transparency log) are each
// optional; leaving both nil selects local-only mode, where every cycle and
// every mutation gate still verify the complete local chain but no external
// service is ever contacted. A Witness requires rollback-external Continuity.
...
func NewManager(config ManagerConfig) (*Manager, error) {
	if config.Store == nil {
		return nil, errors.New("audit anchor: store is required")
	}
	if config.Witness != nil && config.Continuity == nil {
		return nil, errors.New("audit anchor: an external witness requires rollback-external continuity")
	}
...
// database writable. Without an external witness only the local hash chain
// (and, when a timestamp authority is configured, the latest persisted signed
// checkpoint) is verified: there is no external history to recover.
func (manager *Manager) VerifyStartup(ctx context.Context) error {
	...
	if hasExisting {
		...
		if manager.authority == nil {
			// Without a timestamp authority no local checkpoint is ever
			// recorded, so a non-genesis chain is normally ahead of its most
			// recent witness. recoverWitnessHistory already proved every
			// completed witness and pending intent binds an exact local chain
			// row; the fail-closed requirement that remains is that at least
			// one rollback-external binding exists at all.
			if len(history) == 0 && pending == nil {
				return fmt.Errorf(
					"audit anchor: non-genesis audit history exists without rollback-external witness evidence "+
						"(chain head %d:%s); if the Rekor witness is being newly enabled on this existing "+
						"deployment, restart once with --accept-witness-genesis=%d:%s "+
						"(helm: auditAnchor.acceptWitnessGenesis)",
					head.ID, hex.EncodeToString(head.SHA256), head.ID, hex.EncodeToString(head.SHA256))
			}
		} else {
			...
			if !pendingFirstCheckpoint && !completedFirstCheckpoint {
				return fmt.Errorf(
					"audit anchor: non-genesis audit history exists without a signed checkpoint or exact rollback-external witness "+
						"(chain head %d:%s); if the Rekor witness is being newly enabled on this existing "+
						"deployment, restart once with --accept-witness-genesis=%d:%s "+
						"(helm: auditAnchor.acceptWitnessGenesis)",
					head.ID, hex.EncodeToString(head.SHA256), head.ID, hex.EncodeToString(head.SHA256))
...
// any network listener or application mutation path is exposed. Without
// external services configured it verifies the complete local hash chain.
...
func (manager *Manager) Ready(ctx context.Context) error {
	...
	manager.mu.RUnlock()
	if !checked {
		return errors.New("audit anchor: initial verification cycle has not completed")
	}
	if checkErr != nil {
		return checkErr
	}
	if manager.authority == nil {
		// Without a timestamp authority no signed checkpoint exists whose age
		// could go stale; a verified cycle result is the readiness signal.
		return nil
	}
...
// would leave a window between periodic anchor cycles. Without an external
// witness there is no rollback-external history to refresh; the gate then
// re-verifies the complete local hash chain (and, when a timestamp authority
// is configured, the persisted signed checkpoint tip) before every mutation.
func (manager *Manager) AllowMutation(ctx context.Context) error {
	...
	if manager.witness == nil {
		return manager.allowMutationWithoutWitness(ctx)
	}
	...
	manager.mu.RUnlock()
	verifyMutationState := manager.store.VerifyAuditMutationState
	if manager.authority == nil {
		verifyMutationState = manager.store.VerifyAuditMutationStateUnanchored
	}
	var gateErr error
	_, _, err = verifyMutationState(ctx, intents, history, func(_ model.AuditHead, persisted model.AuditAnchor) error {
		if manager.authority != nil {
...
// allowMutationWithoutWitness is the mutation gate for deployments with no
// external witness configured. The complete local hash chain is re-verified
// immediately before the mutation; when a timestamp authority is configured
// the persisted signed checkpoint tip must additionally verify and match the
// in-process verified tip.
func (manager *Manager) allowMutationWithoutWitness(ctx context.Context) error {
	sinceID, sinceDigest := manager.getVerifiedHead()
	if _, err := manager.store.VerifyAuditStateSince(ctx, sinceID, sinceDigest); err != nil {
		return fmt.Errorf("audit anchor: mutation gate verify local protected state: %w", err)
	}
	if manager.authority == nil {
		return nil
	}
	persisted, err := manager.store.LatestAuditAnchor(ctx)
	if err != nil {
		return fmt.Errorf("audit anchor: mutation gate persisted checkpoints: %w", err)
	}
	if err := manager.authority.Verify(persisted); err != nil {
	...
	manager.mu.RUnlock()
	if !sameAuditAnchor(persisted, cachedAnchor) {
		return errors.New("audit anchor: mutation gate persisted checkpoint tip differs from the verified in-process tip")
	}
	return nil
}
...
func (manager *Manager) cycle(ctx context.Context) {
	...
	if manager.witness == nil {
		// The complete local chain is verified and no external history exists
		// to recover; without a timestamp authority the cycle ends here.
		manager.markWitnessHistoryVerified()
		if manager.authority == nil {
			manager.setResult(model.AuditAnchor{}, nil)
			return
		}
		spec, err := manager.authority.Timestamp(ctx, head)
		if err != nil {
		...
		manager.recordVerifiedCheckpoint(ctx, spec)
		return
	}
	...
	var spec model.AuditAnchorSpec
	if manager.authority != nil {
...
// recordVerifiedCheckpoint persists a fresh signed checkpoint and re-verifies
// the stored copy before it becomes the readiness anchor. Callers must hold
// refreshMu and must only invoke this when a timestamp authority is configured.
func (manager *Manager) recordVerifiedCheckpoint(ctx context.Context, spec model.AuditAnchorSpec) {
...
func (manager *Manager) setVerifiedHead(head model.AuditHead) {
	manager.mu.Lock()
	defer manager.mu.Unlock()
	manager.verifiedHead = head.ID
	manager.verifiedDigest = append(manager.verifiedDigest[:0], head.SHA256...)
}

func (manager *Manager) getVerifiedHead() (int64, []byte) {
	manager.mu.RLock()
	defer manager.mu.RUnlock()
	if manager.verifiedHead <= 0 || len(manager.verifiedDigest) == 0 {
		return 0, nil
	}
	return manager.verifiedHead, append([]byte(nil), manager.verifiedDigest...)
}
