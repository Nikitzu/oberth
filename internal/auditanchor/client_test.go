...
func TestNewClientRejectsHTTPWithoutInsecureFlag(t *testing.T) {
	t.Parallel()
	_, err := NewClient(ClientConfig{Endpoint: "http://tsa.example.test/ts", Roots: x509.NewCertPool()})
	if err == nil {
		t.Fatal("plain-HTTP TSA endpoint accepted without InsecureHTTP flag")
	}
	if !strings.Contains(err.Error(), "HTTPS") {
		t.Fatalf("error should mention HTTPS requirement, got: %v", err)
	}
}

func TestNewClientAcceptsHTTPWithInsecureFlag(t *testing.T) {
	t.Parallel()
	client, err := NewClient(ClientConfig{
		Endpoint: "http://tsa.example.test/ts", Roots: x509.NewCertPool(), InsecureHTTP: true,
	})
	if err != nil {
		t.Fatalf("plain-HTTP TSA endpoint rejected with InsecureHTTP=true: %v", err)
	}
	if client == nil {
...
func TestNewClientAcceptsHTTPSRegardlessOfInsecureFlag(t *testing.T) {
	t.Parallel()
	for _, insecure := range []bool{false, true} {
		client, err := NewClient(ClientConfig{
			Endpoint: "https://tsa.example.test/ts", Roots: x509.NewCertPool(), InsecureHTTP: insecure,
		})
		if err != nil {
			t.Fatalf("HTTPS TSA endpoint rejected with InsecureHTTP=%v: %v", insecure, err)
		}
		if client == nil {
			t.Fatal("client is nil")
		}
...
func TestClientObtainsAndVerifiesRFC3161Checkpoint(t *testing.T) {
	...
	client, err := NewClient(ClientConfig{
		...
		InsecureHTTP: true, // httptest.NewServer serves plain HTTP
	})
	if err != nil {
		t.Fatal(err)
	}
...
var codeSigningUsageOID = asn1.ObjectIdentifier{1, 3, 6, 1, 5, 5, 7, 3, 3}

func TestClientRejectsMixedPurposeEKU(t *testing.T) {
	t.Parallel()
	now := time.Date(2026, 8, 6, 14, 0, 0, 0, time.UTC)
	roots, _, signingKey, chain := testTSAIdentity(t, now)
	...
	rootTemplate := &x509.Certificate{
		SerialNumber: big.NewInt(100), Subject: pkix.Name{CommonName: "Mixed EKU test root"},
	...
	mixedRoot, err := x509.ParseCertificate(rootDER)
	if err != nil {
		t.Fatal(err)
	}
	mixedRoots := x509.NewCertPool()
	mixedRoots.AddCert(mixedRoot)

	mixedUsageValue, err := asn1.Marshal([]asn1.ObjectIdentifier{timestampingUsageOID, codeSigningUsageOID})
	if err != nil {
		t.Fatal(err)
	}
	mixedKey, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		t.Fatal(err)
	}
	mixedTemplate := &x509.Certificate{
		SerialNumber: big.NewInt(101), Subject: pkix.Name{CommonName: "Mixed EKU signer"},
		NotBefore: validityStart, NotAfter: validityEnd,
		KeyUsage:    x509.KeyUsageDigitalSignature,
		ExtKeyUsage: []x509.ExtKeyUsage{x509.ExtKeyUsageTimeStamping, x509.ExtKeyUsageCodeSigning},
		ExtraExtensions: []pkix.Extension{{
			Id: extendedKeyUsageOID, Critical: true, Value: mixedUsageValue,
		}},
	}
	mixedDER, err := x509.CreateCertificate(rand.Reader, mixedTemplate, mixedRoot, &mixedKey.PublicKey, rootKey)
	if err != nil {
		t.Fatal(err)
	}
	mixedCert, err := x509.ParseCertificate(mixedDER)
	if err != nil {
		t.Fatal(err)
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		response, err := (&timestamp.Timestamp{
			HashAlgorithm: query.HashAlgorithm, HashedMessage: query.HashedMessage,
			Time: now, Nonce: query.Nonce, Policy: asn1.ObjectIdentifier{1, 2, 3, 4},
			AddTSACertificate: true, Certificates: []*x509.Certificate{mixedRoot},
		}).CreateResponseWithOpts(mixedCert, mixedKey, crypto.SHA256)
		if err != nil {
			t.Error(err)
			return
		}
	...
	client, err := NewClient(ClientConfig{
		Endpoint: server.URL, Roots: mixedRoots, HTTPClient: server.Client(),
	...
	hash := sha256.Sum256([]byte("audit head"))
	_, err = client.Timestamp(context.Background(), model.AuditHead{ID: 42, SHA256: hash[:]})
	if err == nil {
		t.Fatal("mixed-purpose EKU certificate was accepted by RFC 3161 verifier")
	}
	if !strings.Contains(err.Error(), "sole EKU per RFC 3161") {
