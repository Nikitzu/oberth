...
func TestParseRekorEndpointAllowsHTTPOnlyWhenOptedIn(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name         string
		endpoint     string
		...
		{name: "HTTPS by default", endpoint: "https://rekor.example.test/path", wantScheme: "https"},
		{name: "HTTPS with opt-in", endpoint: "https://rekor.example.test/path", insecureHTTP: true, wantScheme: "https"},
		{name: "HTTP by default", endpoint: "http://rekor.example.test", wantError: true},
		{name: "HTTP with opt-in", endpoint: "http://rekor.example.test", insecureHTTP: true, wantScheme: "http"},
		{name: "credentials with opt-in", endpoint: "http://user:pass@rekor.example.test", insecureHTTP: true, wantError: true},
		{name: "fragment with opt-in", endpoint: "http://rekor.example.test/path#fragment", insecureHTTP: true, wantError: true},
		{name: "other scheme with opt-in", endpoint: "file:///tmp/rekor", insecureHTTP: true, wantError: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			parsed, err := parseRekorEndpoint(test.endpoint, test.insecureHTTP)
			if test.wantError {
				if err == nil {
					t.Fatalf("parseRekorEndpoint(%q, %t) succeeded", test.endpoint, test.insecureHTTP)
				}
				return
			}
			if err != nil {
				t.Fatalf("parseRekorEndpoint(%q, %t): %v", test.endpoint, test.insecureHTTP, err)
			}
			if parsed.Scheme != test.wantScheme {
				t.Fatalf("scheme = %q, want %q", parsed.Scheme, test.wantScheme)
			}
		})
	}
}
...
func TestRekorPublicKeyVerifierAcceptsLocalEntryAndRejectsWrongKey(t *testing.T) {
	privateKey, err := ecdsa.GenerateKey(elliptic.P256(), cryptorand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	entry := signedLocalRekorEntry(t, privateKey)
	verifier, err := newRekorPublicKeyVerifier(&privateKey.PublicKey)
	if err != nil {
		t.Fatal(err)
	}
	if err := verifier.Verify(context.Background(), &entry); err != nil {
		t.Fatalf("entry signed by configured local Rekor key failed verification: %v", err)
	}

	wrongKey, err := ecdsa.GenerateKey(elliptic.P256(), cryptorand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	wrongVerifier, err := newRekorPublicKeyVerifier(&wrongKey.PublicKey)
	if err != nil {
		t.Fatal(err)
	}
	if err := wrongVerifier.Verify(context.Background(), &entry); err == nil {
		t.Fatal("entry signed by a different local Rekor key verified")
	}
}

func TestNewRekorWitnessUsesConfiguredPublicKeyWithoutTUF(t *testing.T) {
	t.Setenv("TUF_ROOT", t.TempDir())
	privateKey, err := ecdsa.GenerateKey(elliptic.P256(), cryptorand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	witness, err := NewRekorWitness(context.Background(), []byte("persistent host key"), RekorWitnessConfig{
		Endpoint:  "https://rekor.example.test",
		PublicKey: &privateKey.PublicKey,
	})
	if err != nil {
		t.Fatal(err)
	}
	verifier, ok := witness.verifier.(*rekorTrustedKeyVerifier)
	if !ok || verifier.trustSource != "configured Rekor public key" {
		t.Fatalf("configured witness verifier = %#v", witness.verifier)
	}
}

func signedLocalRekorEntry(t *testing.T, privateKey *ecdsa.PrivateKey) models.LogEntryAnon {
	t.Helper()
	ctx := context.Background()
	signer, err := signature.LoadSignerVerifier(privateKey, crypto.SHA256)
	if err != nil {
		t.Fatal(err)
	}
	...
	logIDHash := sha256.Sum256(publicDER)
	logID := hex.EncodeToString(logIDHash[:])
	body := []byte(`{"apiVersion":"0.0.1","kind":"hashedrekord","spec":{}}`)
	rootHash := rfc6962.DefaultHasher.HashLeaf(body)
	rootHashHex := hex.EncodeToString(rootHash)
	integratedAt := time.Now().Unix()
	logIndex := int64(0)
	treeSize := int64(1)
	entry := models.LogEntryAnon{
		Body:           base64.StdEncoding.EncodeToString(body),
		IntegratedTime: &integratedAt,
		LogID:          &logID,
		LogIndex:       &logIndex,
	}
	...
	signedEntryTimestamp, err := signer.SignMessage(bytes.NewReader(canonicalPayload), options.WithContext(ctx))
	if err != nil {
		t.Fatal(err)
	}
	checkpoint, err := util.CreateAndSignCheckpoint(ctx, "rekor.test", 1, uint64(treeSize), rootHash, signer)
	if err != nil {
		t.Fatal(err)
	}
	checkpointText := string(checkpoint)
	entry.Verification = &models.LogEntryAnonVerification{
		InclusionProof: &models.InclusionProof{
...
func TestAbandonedChainTipReportsAbsentHistoryAndFailsClosed(t *testing.T) {
	...
	for index := range maximumEnumerationCandidates + 1 {
		api.byEmail[witness.identity] = append(api.byEmail[witness.identity], fmt.Sprintf("%064x", index))
	}
	if _, _, err := witness.AbandonedChainTip(ctx); err == nil || !strings.Contains(err.Error(), "enumeration budget") {
		t.Fatalf("identity flood error = %v, want enumeration budget error", err)
	}
}

func TestAbandonedChainTipFiltersForeignEntriesBeforeOwnLimit(t *testing.T) {
	ctx := context.Background()
	api := newMemoryRekorAPI()
	verifier := &fakeRekorEntryVerifier{}
	keyMaterial := []byte("persistent host key")
	...
	hash := sha256.Sum256([]byte("own head"))
	head := model.AuditHead{ID: 1, SHA256: hash[:]}
	if _, err := publisher.History(ctx, nil, head); err != nil {
		t.Fatal(err)
	}
	own, err := publisher.Publish(ctx, head)
	if err != nil {
		t.Fatal(err)
	}

	foreign, err := newRekorWitness(api, verifier, []byte("foreign key"))
	if err != nil {
		t.Fatal(err)
	}
	for index := range maximumIdentitySearchResults + 10 {
		foreignHash := sha256.Sum256([]byte(fmt.Sprintf("foreign %d", index)))
		foreignHead := model.AuditHead{ID: int64(index + 1), SHA256: foreignHash[:]}
		if index == 0 {
			if _, err := foreign.History(ctx, nil, foreignHead); err != nil {
				t.Fatal(err)
			}
		}
		entry, err := foreign.Publish(ctx, foreignHead)
		if err != nil {
			t.Fatal(err)
		}
		api.byEmail[publisher.identity] = append(api.byEmail[publisher.identity], entry.UUID)
	}
	...
	tip, found, err := fresh.AbandonedChainTip(ctx)
	if err != nil {
		t.Fatalf("abandoned tip with foreign entries: %v", err)
	}
	if !found || !sameAuditWitness(tip, own) {
		t.Fatalf("tip = %#v found=%v, want own entry %#v", tip, found, own)
	}
}
