...
type ClientConfig struct {
	...
	// InsecureHTTP permits a plain-HTTP endpoint for DEVELOPMENT ONLY TSA
...
func NewClient(config ClientConfig) (*Client, error) {
	...
	validScheme := endpoint != nil && (endpoint.Scheme == "https" || config.InsecureHTTP && endpoint.Scheme == "http")
	if err != nil || strings.Contains(config.Endpoint, "#") || endpoint == nil || endpoint.Host == "" || !validScheme || endpoint.User != nil || endpoint.Fragment != "" ||
		(endpoint.Scheme == "https" && !strings.HasPrefix(config.Endpoint, "https://")) ||
		(endpoint.Scheme == "http" && !strings.HasPrefix(config.Endpoint, "http://")) {
		requiredScheme := "HTTPS"
		if config.InsecureHTTP {
			requiredScheme = "HTTP or HTTPS"
		}
		return nil, fmt.Errorf("audit anchor: TSA endpoint must be an absolute %s URL without credentials or a fragment", requiredScheme)
	}
	if config.Roots == nil {
...
func (client *Client) verifyReceipt(receipt, expectedHash []byte) (*timestamp.Timestamp, error) {
	...
	if len(signer.ExtKeyUsage) != 1 || signer.ExtKeyUsage[0] != x509.ExtKeyUsageTimeStamping ||
		len(signer.UnknownExtKeyUsage) != 0 || !criticalExtendedKeyUsage(signer) {
		return nil, errors.New("audit anchor: signer certificate must carry exactly id-kp-timeStamping as its sole EKU per RFC 3161")
	}
