...
type Config struct {
	...
	// SessionHandler, when set, supersedes OnUpdate for live receive
	// sessions: it receives a notice sink whose lines reach the pushing
	// client as remote: output on SSH stderr, and returns the receive
	// handler for exactly that session. Startup replay always uses OnUpdate
	// directly — a replayed event has no attached client terminal.
	SessionHandler func(notify func(format string, args ...any)) UpdateHandler
	// PushBannerURL, when non-empty, is echoed after every accepted push as
	// "remote: watch: <url>" so the pusher can follow the queued run.
...
func New(config Config) (*Server, error) {
	...
	return &Server{
		...
		pushBannerURL:            config.PushBannerURL,
...
func (s *Server) runGit(ctx context.Context, actor, protocol string, command gitCommand, channel ssh.Channel) error {
	...
	if command.service == gitcache.ReceivePack {
		...
		// Git clients forward the remote side's raw SSH stderr, so these
		// lines reach the pusher's terminal exactly as written. stderr is a
		// separate SSH extended-data stream: writing here can never corrupt
		// the receive-pack protocol on stdout/stdin.
		stderr := channel.Stderr()
		notify := func(format string, args ...any) {
			_, _ = fmt.Fprintf(stderr, "remote: "+format+"\n", args...)
		}
		notify("[oberth] uplink %s", actor)
		handler := s.onUpdate
		if s.sessionHandler != nil {
			handler = s.sessionHandler(notify)
		}
		guardedHandler := UpdateHandlerFunc(func(ctx context.Context, event gitcache.ReceiveEvent) error {
			if err := s.mutationGate(ctx); err != nil {
				return fmt.Errorf("audit integrity check after pack upload: %w", err)
			}
			return handler.HandleReceive(ctx, event)
		})
		...
		if err := s.git.Receive(ctx, repoInput, actor, protocol, channel, channel, stderr, guardedHandler); err != nil {
			return err
		}
		if s.pushBannerURL != "" {
			notify("watch: %s", s.pushBannerURL)
		}
		return nil
	}
	...
	return s.git.Serve(ctx, repoInput, command.service, protocol, channel, channel, channel.Stderr())
}
