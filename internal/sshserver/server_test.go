...
func TestOrgQualifiedPathResolvesToBareRepo(t *testing.T) {
	t.Parallel()
	clientSigner := newSigner(t)
	git := &fakeGit{refs: map[string]string{"refs/heads/main": shaA}}
	...
	if err := session.Run("git-upload-pack '/acme/example.git'"); err != nil {
		t.Fatal(err)
	}
	git.mu.Lock()
	defer git.mu.Unlock()
	if git.service != gitcache.UploadPack || git.repo != "example" {
		t.Fatalf("org-qualified clone: service %q repo %q, want upload-pack + example", git.service, git.repo)
	}
}
...
// TestAuditGateReCheckedAfterPackUploadBeforeCallbacks proves that the
// sshserver's guardedHandler wrapper re-checks the audit mutation gate
// before each callback delivery. The pre-finalize gate (which fires
// before ref publication) lives in the gitcache.Cache layer and is
// tested by TestPreFinalizeGateBlocksRefPublication in the gitcache
// package; this test covers the per-callback layer that applies to both
// live receives and replayed reservations.
func TestAuditGateReCheckedAfterPackUploadBeforeCallbacks(t *testing.T) {
	t.Parallel()
	clientSigner := newSigner(t)
	git := &fakeGit{
		refs:         map[string]string{"refs/heads/main": shaA},
	...
	server, err := New(Config{
		...
		Git: git,
		OnUpdate: UpdateHandlerFunc(func(context.Context, gitcache.ReceiveEvent) error {
			return nil
		}),
	...
	if err := session.Run("git-receive-pack '/example.git'"); err == nil {
		t.Fatal("receive-pack passed while the per-callback audit gate failed")
	}
	if !strings.Contains(stderr.String(), "audit integrity check after pack upload") {
		t.Fatalf("expected per-callback audit gate error, got: %q", stderr.String())
	}
	if calls := gateCalls.Load(); calls < 2 {
		t.Fatalf("mutation gate was called %d times, want at least 2 (early + per-callback)", calls)
	}
}
...
func (f *fakeGit) Ensure(_ context.Context, input string) (gitcache.Repository, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.ensureCalls++
...
func TestReceiveSessionEmitsRemoteBannerAndSessionNotices(t *testing.T) {
	t.Parallel()
	clientSigner := newSigner(t)
	git := &fakeGit{
		refs:         map[string]string{"refs/heads/main": shaA},
	...
	var onUpdateCalls atomic.Int32
	server, err := New(Config{
		HostSigner: newSigner(t),
		Resolver: IdentityResolverFunc(func(fingerprint string) (string, bool, error) {
			return "worker@tuxbox", fingerprint == ssh.FingerprintSHA256(clientSigner.PublicKey()), nil
		...
		SessionHandler: func(notify func(format string, args ...any)) UpdateHandler {
			return UpdateHandlerFunc(func(_ context.Context, event gitcache.ReceiveEvent) error {
				notify("ref accepted · run queued (%s)", event.Repo)
				return nil
			})
		},
		PushBannerURL: "https://watch.example/runs",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	uplink := strings.Index(output, "remote: [oberth] uplink worker@tuxbox\n")
	notice := strings.Index(output, "remote: ref accepted · run queued (example)\n")
	watch := strings.Index(output, "remote: watch: https://watch.example/runs\n")
	if uplink < 0 || notice < 0 || watch < 0 {
		t.Fatalf("push feedback missing lines: %q", output)
	}
	if uplink >= notice || notice >= watch {
		t.Fatalf("push feedback out of order: %q", output)
	}
	if calls := onUpdateCalls.Load(); calls != 0 {
		t.Fatalf("session handler was configured but OnUpdate ran %d times", calls)
	}
}

func TestReceiveFailureSuppressesWatchLine(t *testing.T) {
	t.Parallel()
	clientSigner := newSigner(t)
	git := &fakeGit{
		refs:         map[string]string{"refs/heads/main": shaA},
	...
	server, err := New(Config{
		...
		Git: git,
		OnUpdate: UpdateHandlerFunc(func(context.Context, gitcache.ReceiveEvent) error {
			return errors.New("durable delivery failed")
		}),
		PushBannerURL: "https://watch.example/runs",
	})
	if err != nil {
	...
	session.Stderr = &stderr
	if err := session.Run("git-receive-pack '/example.git'"); err == nil {
		t.Fatal("failed receive returned success")
	}
	output := stderr.String()
	if !strings.Contains(output, "remote: [oberth] uplink worker@tuxbox\n") {
		t.Fatalf("identity banner missing: %q", output)
	}
	if strings.Contains(output, "remote: watch:") {
		t.Fatalf("failed receive still advertised the watch URL: %q", output)
	}
	if !strings.Contains(output, "durable delivery failed") {
		t.Fatalf("delivery error not surfaced to the pusher: %q", output)
	}
}
