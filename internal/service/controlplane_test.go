...
func (git *controlGit) Checkout(_ context.Context, _ string, sha, destination string) error {
	...
	for relative, body := range git.checkoutFiles {
...
func (git *controlGit) PushPromotion(ctx context.Context, repo, target, sha string) error {
	if git.events != nil {
		*git.events = append(*git.events, "publish")
	}
...
type fakeJobs struct {
	...
	createdPlans    []JobRequest
	createdApplies  []JobRequest
	deleted         []string
	size            periapsis.Size
	plan            []runprogress.PlannedStep
	planErr         error
}
...
func (jobs *fakeJobs) PipelineSize(context.Context, JobRequest) (periapsis.Size, error) {
...
func (jobs *fakeJobs) CreatePlan(_ context.Context, request JobRequest) error {
	jobs.mu.Lock()
	defer jobs.mu.Unlock()
	jobs.createdPlans = append(jobs.createdPlans, request)
	return nil
}

func (jobs *fakeJobs) CreateApply(_ context.Context, request JobRequest) error {
	jobs.mu.Lock()
	defer jobs.mu.Unlock()
	jobs.createdApplies = append(jobs.createdApplies, request)
	return nil
}
...
func TestSchedulerPassesStaticSizeIntoJobCreation(t *testing.T) {
	fixture := newControlFixture(t, JobResult{
		Status: model.RunPassed, Phase: "passed", Steps: []model.StepResult{stepResult(model.StepPassed)},
	})
	fixture.jobs.size = periapsis.L
	enqueued, err := fixture.scheduler.EnqueueCI(context.Background(), CIRequest{
		EventID: "sized-run", Repository: fixture.repo, Branch: "feature/sized",
	...
	if len(fixture.jobs.createdCI) != 1 || fixture.jobs.createdCI[0].Run.ID != enqueued.ID || fixture.jobs.createdCI[0].Size != periapsis.L {
		t.Fatalf("created requests = %#v", fixture.jobs.createdCI)
	}
}
	...
	for index, sha := range shas {
		...
		if index < 2 {
			if !strings.Contains(page.Issues[0].Body, sha) ||
				...
				!strings.Contains(page.Issues[0].Body, "full step log: run_logs ") ||
				!strings.Contains(page.Issues[0].Body, " test unit") ||
...
func TestDashboardRunDetailAndLogViewsAreReadOnly(t *testing.T) {
	...
	mcpDetailValue, err := control.CallTool(ctx, api.Actor{Identity: "agent@host"}, "run_get",
		json.RawMessage(`{"id":"`+enqueued.ID+`"}`))
	if err != nil {
		t.Fatal(err)
	}
	mcpDetail := mcpDetailValue.(RunDetailResponse)
	if mcpDetail.Run.ID != enqueued.ID || len(mcpDetail.Steps) != 2 {
		t.Fatalf("run_get response = %#v", mcpDetail)
	}
	mcpLogValue, err := control.CallTool(ctx, api.Actor{Identity: "agent@host"}, "run_logs",
		json.RawMessage(`{"id":"`+enqueued.ID+`","burn":"test","step":"unit"}`))
	if err != nil {
		t.Fatal(err)
	}
	mcpLog := mcpLogValue.(LogResponse)
	if mcpLog.RunID != enqueued.ID || mcpLog.Burn != "test" || mcpLog.Step != "unit" || !strings.Contains(mcpLog.Output, "[test/unit]") {
		t.Fatalf("run_logs response = %#v", mcpLog)
	}
	if _, err := control.CallTool(ctx, api.Actor{Identity: "agent@host"}, "run_logs",
		json.RawMessage(`{"id":"`+enqueued.ID+`","burn":"lint","step":"unit"}`)); err == nil {
		t.Fatal("run_logs accepted a mismatched exact burn/step pair")
	}
}

func TestActiveRunDetailAndStatusExposePersistedStepProgress(t *testing.T) {
	fixture := newControlFixture(t)
	const sha = "fdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfd"
	ctx := context.Background()
	enqueued, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "receive-live-step-plan", Repository: fixture.repo,
		Branch: "feature/live-step-plan", SHA: sha, Actor: "agent@host",
	})
	if err != nil {
		t.Fatal(err)
	}
	running, err := fixture.store.ClaimNextRun(ctx)
	if err != nil {
		t.Fatal(err)
	}
	if running.ID != enqueued.ID || running.Status != model.RunRunning {
		t.Fatalf("claimed run = %#v", running)
	}
	journal := strings.Join([]string{
		`{"version":1,"burn":"test","step":"unit","ordinal":0,"status":"running","declared_size":"M","started_at":"2026-08-11T12:00:00Z"}`,
		`{"version":1,"burn":"release","step":"publish","ordinal":1,"status":"queued","declared_size":"L"}`,
		"",
	}, "\n")
	...
	value, err := control.Run(ctx, api.Actor{Identity: "dashboard@viewer"}, enqueued.ID)
	if err != nil {
		t.Fatal(err)
	}
	detail := value.(RunDetailResponse)
	if len(detail.Steps) != 2 || detail.Steps[0].Burn != "test" || string(detail.Steps[0].Status) != "running" ||
		detail.Steps[1].Burn != "release" || string(detail.Steps[1].Status) != "queued" {
		t.Fatalf("active run detail steps = %#v", detail.Steps)
	}
	status, err := control.status(ctx, fixture.repo.Name, sha, "")
	if err != nil {
		t.Fatal(err)
	}
	if status.Burns["test"] != "running" || status.Burns["release"] != "queued" {
		t.Fatalf("active run burn states = %#v", status.Burns)
	}
}

func TestActiveMCPAndDashboardLogsReadCurrentStepRange(t *testing.T) {
	fixture := newControlFixture(t)
	const sha = "cdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcdcd"
	ctx := context.Background()
	enqueued, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "receive-active-step-log", Repository: fixture.repo,
		Branch: "feature/active-step-log", SHA: sha, Actor: "agent@host",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	logFile, err := fixture.logs.Create(enqueued.ID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := io.WriteString(logFile,
		"[setup/prepare] unrelated before\n"+
			"[release/advance-stable-aliases] $ publish alias\n"+
			"[release/advance-stable-aliases] publishing current digest\n"+
			"[verify/other] unrelated after\n"); err != nil {
		t.Fatal(err)
	}
	...
	if err := fixture.logs.AppendStepProgress(enqueued.ID, runprogress.Event{
		Version: runprogress.Version, Burn: "release", Step: "advance-stable-aliases", Ordinal: 0,
	...
	value, err := control.CallTool(ctx, api.Actor{Identity: "agent@host"}, "logs", json.RawMessage(
		`{"repo":"oberth","sha":"`+sha+`","step":"advance-stable-aliases"}`,
	))
	if err != nil {
		t.Fatal(err)
	}
	mcpLog := value.(LogResponse)
	if !strings.Contains(mcpLog.Output, "publishing current digest") || strings.Contains(mcpLog.Output, "unrelated") {
		t.Fatalf("active MCP log = %q", mcpLog.Output)
	}
	value, err = control.RunLog(ctx, api.Actor{Identity: "dashboard@viewer"}, enqueued.ID, "release", "advance-stable-aliases")
	if err != nil {
		t.Fatal(err)
	}
	dashboardLog := value.(LogResponse)
	if dashboardLog.Output != mcpLog.Output {
		t.Fatalf("dashboard log = %q, MCP log = %q", dashboardLog.Output, mcpLog.Output)
	}
}
...
func TestSyncRejectsBareSHAWhenMultipleBranchesShareIt(t *testing.T) {
	...
	// Enqueue runs on two different branches sharing the same SHA.
	if _, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "receive-branch-a", Repository: fixture.repo, Branch: "feature/alpha",
		SHA: sha, Actor: "agent@host",
	}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "receive-branch-b", Repository: fixture.repo, Branch: "feature/beta",
		SHA: sha, Actor: "agent@host",
	}); err != nil {
		t.Fatal(err)
	}
	...
	// Bare-SHA sync should be rejected because both branches share the SHA.
	_, err := service.CallTool(ctx, api.Actor{Identity: "agent@host"}, "sync", json.RawMessage(`{"sha":"`+sha+`"}`))
	if err == nil {
		t.Fatal("bare-SHA sync accepted despite multiple candidate branches")
	}
	if !strings.Contains(err.Error(), "multiple branches") {
		t.Fatalf("error = %v, want mention of multiple branches", err)
	}
	if !strings.Contains(err.Error(), "feature/alpha") || !strings.Contains(err.Error(), "feature/beta") {
		t.Fatalf("error should list both candidate branches: %v", err)
	}

	// Explicit branch should succeed.
	_, err = service.CallTool(ctx, api.Actor{Identity: "agent@host"}, "sync", json.RawMessage(`{"sha":"`+sha+`","branch":"feature/alpha"}`))
	if err != nil {
		t.Fatalf("explicit branch sync failed: %v", err)
	}

	// Single-branch SHA should still work bare.
	const singleSHA = "ffffffffffffffffffffffffffffffffffffffff"
	if _, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "receive-single", Repository: fixture.repo, Branch: "feature/solo",
		SHA: singleSHA, Actor: "agent@host",
	}); err != nil {
		t.Fatal(err)
	}
	// No need to process — sync only needs the run record.
	_, err = service.CallTool(ctx, api.Actor{Identity: "agent@host"}, "sync", json.RawMessage(`{"sha":"`+singleSHA+`"}`))
	if err != nil {
		t.Fatalf("single-branch bare-SHA sync failed: %v", err)
	}
}
...
// TestJobObservationCancellationDoesNotImpersonateSchedulerShutdown verifies
// that a Wait error from an inner observation context (context.Canceled,
// context.DeadlineExceeded) is recorded as RunFailed — NOT RunInterrupted,
// which is the scheduler-shutdown pattern. The workflow IS deleted before
// terminalizing (Fix 2: a still-running workflow must not be left untracked
// while its durable run is recorded as terminal).
func TestJobObservationCancellationDoesNotImpersonateSchedulerShutdown(t *testing.T) {
	for name, waitErr := range map[string]error{
		...
		t.Run(name, func(t *testing.T) {
			...
			// The run must be terminalized as RunFailed (not RunInterrupted,
			// which is the scheduler shutdown pattern).
			...
			// The workflow must be deleted before terminalizing: a
			// still-running workflow must not be left untracked while its
			// durable run is recorded as terminal.
			if len(fixture.jobs.deleted) != 1 {
				t.Fatalf("expected exactly one Job deletion before terminalizing, got %v", fixture.jobs.deleted)
			}
		})
	}
}
...
func TestReleaseChecksOutCommitButPublishesRawTagObject(t *testing.T) {
	...
	// The scheduler binds the registered upstream's org identity to every
	// release JobRequest so hierarchical secret path scoping can match
	// oberth/upstream/<org>/... declarations against it (the fixture upstream
	// base URL is ssh://codeberg.org/acme).
	if fixture.jobs.createdReleases[0].UpstreamOrg != "acme" {
		t.Fatalf("release upstream org = %q, want %q", fixture.jobs.createdReleases[0].UpstreamOrg, "acme")
	}
}
...
func TestAdmittedPromotionAlwaysReturnsDurableID(t *testing.T) {
	...
	// The "pre-enqueue audit failure" and "blocked audit is a scheduler
	// admission barrier" subtests were removed: the promotion.pending audit
	// is now recorded atomically inside Store.PlanPromotion's own
	// transaction, so there is no longer a separate service-level audit call
	// that can fail between planning and enqueue. The in-transaction audit is
	// exercised by TestPlanPromotionRecordsAuditAtomically in the store
	// package.
}
...
func TestAdmittedPromotionCompensationSurvivesRequestCancellation(t *testing.T) {
	...
	// The "audit" subtest was removed: the promotion.pending audit is now
	// inside Store.PlanPromotion's transaction and does not pass through the
	// API's Auditor interface, so a blockingPromotionAuditor at the service
	// level never blocks.
...
func TestStatusUsesDirectIssueLookupWithoutRenewal(t *testing.T) {
	fixture := newControlFixture(t, JobResult{
	...
	// Status must succeed even when RenewIssueLock is broken, because the
	// read-only status path no longer renews locks (#51 defect 1 fix).
	failing := &issueRenewFailer{Store: fixture.store, err: errors.New("renew must not be called from status")}
	service, err = NewAPI(APIConfig{
		Runs: fixture.store, History: fixture.store, Repositories: fixture.store,
		Issues: failing, Promotions: fixture.store,
		PromotionRuns: fixture.store, Enqueues: fixture.scheduler, Git: fixture.git, Logs: fixture.logs,
		Auditor: fixture.store, Signals: fixture.signals,
		PromotionWorkspaceRoot: filepath.Join(fixture.root, "status-no-renew"),
	})
	if err != nil {
		t.Fatal(err)
	}
	value, err = service.CallTool(ctx, api.Actor{Identity: "agent@host"}, "status",
		json.RawMessage(`{"ref":"feature/direct"}`))
	if err != nil {
		t.Fatalf("status should succeed without renewal: %v", err)
	}
	if status := value.(StatusResponse); status.Issue == nil {
		t.Fatalf("status should still report the issue: %#v", status)
	}
	if failing.calls != 0 {
		t.Fatalf("RenewIssueLock was called %d times from status, want 0", failing.calls)
	}

	// issue_get must also succeed without calling RenewIssueLock.
	issue, err := fixture.store.CreateManualIssue(ctx, "agent@host", model.ManualIssueSpec{Title: "manual", Body: "body"})
	if err != nil {
		t.Fatal(err)
	}
	failing2 := &issueRenewFailer{Store: fixture.store, err: errors.New("renew must not be called from issue_get")}
	service, err = NewAPI(APIConfig{
		Runs: fixture.store, History: fixture.store, Repositories: fixture.store,
		Issues: failing2, Promotions: fixture.store,
		PromotionRuns: fixture.store, Enqueues: fixture.scheduler, Git: fixture.git, Logs: fixture.logs,
		Auditor: fixture.store, Signals: fixture.signals,
		PromotionWorkspaceRoot: filepath.Join(fixture.root, "issue-no-renew"),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil {
		t.Fatalf("issue_get should succeed without renewal: %v", err)
	}
	if failing2.calls != 0 {
		t.Fatalf("RenewIssueLock was called %d times from issue_get, want 0", failing2.calls)
	}
}

// TestStatusPollingDoesNotExtendExpiringLock verifies defect 1 of #51: a
// status/wait polling loop must not call RenewIssueLock at all. The lock
// should be renewable only through the explicit issue_lock tool.
func TestStatusPollingDoesNotExtendExpiringLock(t *testing.T) {
	fixture := newControlFixture(t, JobResult{
		Status: model.RunFailed, Phase: "test", FailedBurn: "test", FailedStep: "unit",
		Error: "red", Steps: []model.StepResult{stepResult(model.StepFailed)},
	})
	...
	const sha = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
	if _, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "lock-expiry-poll", Repository: fixture.repo, Branch: "feature/lock-expire",
		SHA: sha, Actor: "agent@host",
	}); err != nil {
		t.Fatal(err)
	}
	...
	// The scheduler already locked the issue for "agent@host" during
	// ProcessNext. Use an issueRenewFailer to prove status never calls
	// RenewIssueLock -- if it did, the test would see a non-zero call count.
	failing := &issueRenewFailer{Store: fixture.store, err: errors.New("renew must not run from status")}
	service, err := NewAPI(APIConfig{
		Runs: fixture.store, History: fixture.store, Repositories: fixture.store,
		...
		Auditor: fixture.store, Signals: fixture.signals,
		PromotionWorkspaceRoot: filepath.Join(fixture.root, "lock-poll"),
	})
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 3; i++ {
		_, err := service.CallTool(ctx, api.Actor{Identity: "agent@host"}, "status",
			json.RawMessage(`{"ref":"feature/lock-expire"}`))
		if err != nil {
			t.Fatalf("status call %d: %v", i, err)
		}
	}
	if failing.calls != 0 {
		t.Fatalf("RenewIssueLock called %d times from status, want 0", failing.calls)
	}
}

// TestMutationGateBlocksIssueLockTool verifies defect 2 of #51: when the
// mutation gate is unavailable, the explicit issue_lock tool must surface the
// error rather than silently succeeding.
func TestMutationGateBlocksIssueLockTool(t *testing.T) {
	fixture := newControlFixture(t)
	ctx := context.Background()
	actor := api.Actor{Identity: "agent@host"}
	issue, err := fixture.store.CreateManualIssue(ctx, actor.Identity, model.ManualIssueSpec{Title: "gate test", Body: "body"})
	if err != nil {
		t.Fatal(err)
	}

	gateErr := errors.New("audit integrity unavailable")
	service, err := NewAPI(APIConfig{
		Runs: fixture.store, History: fixture.store, Repositories: fixture.store,
		Issues: fixture.store, Promotions: fixture.store, PromotionRuns: fixture.store,
		Enqueues: fixture.scheduler, Git: fixture.git, Logs: fixture.logs,
		Auditor: fixture.store, Signals: fixture.signals,
		MutationGate:           func(context.Context) error { return gateErr },
		PromotionWorkspaceRoot: filepath.Join(fixture.root, "gate-lock"),
	})
	if err != nil {
		t.Fatal(err)
	}

	_, err = service.CallTool(ctx, actor, "issue_lock",
		json.RawMessage(fmt.Sprintf(`{"id":%d}`, issue.ID)))
	if err == nil {
		t.Fatal("issue_lock should fail when mutation gate is unavailable")
	}
	if !errors.Is(err, ErrUnavailable) {
		t.Fatalf("issue_lock gate error = %v, want ErrUnavailable", err)
	}
}
...
func TestIssueToolsListBothKindsAndUseActingIdentity(t *testing.T) {
	...
	if locked.ID != created.ID || locked.Owner != actor.Identity || locked.ExpiresAt == "" || !locked.Renewed {
...
func TestRunsViewFiltersRepoAndRef(t *testing.T) {
	fixture := newControlFixture(t, JobResult{
		Status: model.RunPassed, Phase: "passed", Steps: []model.StepResult{stepResult(model.StepPassed)},
	})
	ctx := context.Background()
	actor := api.Actor{Identity: "agent@tuxbox"}
	...
	_, err := fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "push-a", Repository: fixture.repo, Branch: branchA,
		SHA: shaA, Actor: actor.Identity,
	})
	if err != nil {
		t.Fatal(err)
	}
	_, err = fixture.scheduler.EnqueueCI(ctx, CIRequest{
		EventID: "push-b", Repository: fixture.repo, Branch: branchB,
		SHA: shaB, Actor: actor.Identity,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	all, err := service.Runs(ctx, actor, api.RunFilter{Limit: 50})
	if err != nil {
		t.Fatal(err)
	}
	allRuns, ok := all.([]model.Run)
	if !ok || len(allRuns) != 2 {
		t.Fatalf("unfiltered runs = %d %#v", len(allRuns), all)
	}

	// Filter by branchA: only branchA's run.
	onlyA, err := service.Runs(ctx, actor, api.RunFilter{Ref: branchA, Limit: 50})
	if err != nil {
		t.Fatal(err)
	}
	onlyARuns, ok := onlyA.([]model.Run)
	if !ok || len(onlyARuns) != 1 || onlyARuns[0].Ref != branchA {
		t.Fatalf("ref=%q runs = %#v", branchA, onlyA)
	}

	// Filter by repo name.
	byRepo, err := service.Runs(ctx, actor, api.RunFilter{Repository: fixture.repo.Name, Limit: 50})
	if err != nil {
		t.Fatal(err)
	}
	byRepoRuns, ok := byRepo.([]model.Run)
	if !ok || len(byRepoRuns) != 2 {
		t.Fatalf("repo=%q runs = %#v", fixture.repo.Name, byRepo)
	}

	// Filter by repo name and ref together.
	combined, err := service.Runs(ctx, actor, api.RunFilter{Repository: fixture.repo.Name, Ref: branchB, Limit: 50})
	if err != nil {
		t.Fatal(err)
	}
	combinedRuns, ok := combined.([]model.Run)
	if !ok || len(combinedRuns) != 1 || combinedRuns[0].Ref != branchB {
		t.Fatalf("repo+ref runs = %#v", combined)
	}

	// Nonexistent ref: empty result.
	none, err := service.Runs(ctx, actor, api.RunFilter{Ref: "nonexistent", Limit: 50})
	if err != nil {
		t.Fatal(err)
	}
	noneRuns, ok := none.([]model.Run)
	if !ok || len(noneRuns) != 0 {
		t.Fatalf("nonexistent ref runs = %#v", none)
	}
}

// stubSecretAccessStore satisfies SecretAccessStore for admin-gate tests
// without needing a real database or ConfigMap reconciler.
type stubSecretAccessStore struct {
	grants []store.SecretAccessGrant
}

func (s *stubSecretAccessStore) SecretAccessList(_ context.Context, _ string, _ bool) ([]store.SecretAccessGrant, error) {
	return s.grants, nil
}

func (s *stubSecretAccessStore) Grant(_ context.Context, repo, step, secret, actor string) (store.SecretAccessGrant, error) {
	grant := store.SecretAccessGrant{ID: int64(len(s.grants) + 1), Repo: repo, Step: step, Secret: secret, ApprovedBy: actor}
	s.grants = append(s.grants, grant)
	return grant, nil
}

func (s *stubSecretAccessStore) Revoke(_ context.Context, repo, step, secret, actor string) (store.SecretAccessGrant, error) {
	for i := range s.grants {
		if s.grants[i].Repo == repo && s.grants[i].Step == step && s.grants[i].Secret == secret {
			s.grants[i].RevokedBy = actor
...
func TestAccessAllowRequiresAdminUplink(t *testing.T) {
	...
	// Non-admin actor must be rejected before any state read/write.
	_, err = service.CallTool(context.Background(), nonAdmin, "access_allow",
		json.RawMessage(`{"repo":"terraform","step":"*","secret":"terraform/credentials"}`))
	if !errors.Is(err, ErrForbidden) {
		t.Fatalf("access_allow with non-admin error = %v, want ErrForbidden", err)
	}
	// Verify no grant was created.
	if len(secretStore.grants) != 0 {
		t.Fatalf("non-admin access_allow created %d grants, want 0", len(secretStore.grants))
	}

	// Admin actor succeeds.
	admin := api.Actor{Identity: "admin@host", Fingerprint: "SHA256:admin", Admin: true}
	_, err = service.CallTool(context.Background(), admin, "access_allow",
		json.RawMessage(`{"repo":"terraform","step":"*","secret":"terraform/credentials"}`))
	if err != nil {
		t.Fatalf("access_allow with admin error = %v", err)
	}
	if len(secretStore.grants) != 1 {
		t.Fatalf("admin access_allow grants = %d, want 1", len(secretStore.grants))
	}
}

func TestAccessRevokeRequiresAdminUplink(t *testing.T) {
	t.Parallel()
	fixture := newControlFixture(t)
	secretStore := &stubSecretAccessStore{
		grants: []store.SecretAccessGrant{
	...
	_, err = service.CallTool(context.Background(), nonAdmin, "access_revoke",
		json.RawMessage(`{"repo":"terraform","step":"*","secret":"terraform/credentials"}`))
	if !errors.Is(err, ErrForbidden) {
		t.Fatalf("access_revoke with non-admin error = %v, want ErrForbidden", err)
	}

	// Admin actor succeeds.
	admin := api.Actor{Identity: "admin@host", Fingerprint: "SHA256:admin", Admin: true}
	_, err = service.CallTool(context.Background(), admin, "access_revoke",
		json.RawMessage(`{"repo":"terraform","step":"*","secret":"terraform/credentials"}`))
	if err != nil {
		t.Fatalf("access_revoke with admin error = %v", err)
	}
}
...
func TestAuthenticatorAdminPropagation(t *testing.T) {
	...
	// Admin uplink.
	adminAuth, err := NewAuthenticator(stubUplinkAuthenticator{value: model.AuthenticatedUplink{
		Uplink: model.Uplink{Identity: "admin@tuxbox", Fingerprint: "SHA256:admin", Admin: true},
	}})
	if err != nil {
		t.Fatal(err)
	}
	adminActor, err := adminAuth.Authenticate(context.Background(), "admin_token")
	if err != nil {
		t.Fatal(err)
	}
	if !adminActor.Admin {
		t.Fatal("admin uplink should produce Actor.Admin=true")
	}

	// Non-admin uplink.
	regularAuth, err := NewAuthenticator(stubUplinkAuthenticator{value: model.AuthenticatedUplink{
		Uplink: model.Uplink{Identity: "agent@tuxbox", Fingerprint: "SHA256:agent", Admin: false},
	}})
	if err != nil {
		t.Fatal(err)
	}
	regularActor, err := regularAuth.Authenticate(context.Background(), "regular_token")
	if err != nil {
		t.Fatal(err)
	}
	if regularActor.Admin {
