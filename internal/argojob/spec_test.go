...
// oberth/upstream/ path, which no longer needs a grant: that namespace is
// authorized structurally against the pushing repository's own upstream, so a
// grant row could only restate a constraint already enforced. A system path is
// the case the approval table actually decides.
...
// other side, end to end through Build: the org's own subtree needs no
// administrator step, which is what makes `oberth install` plus `oberth init`
// enough to build a repository that pulls a private package.
func TestBuildAdmitsUpstreamScopedPathWithoutAGrant(t *testing.T) {
	withPaths := strings.Replace(greedyDocument, "    oberth.ci/size: L\n",
		"    oberth.ci/size: L\n    oberth.ci/secret-paths: oberth/upstream/skipops/github-token\n", 1)
	request := testRequest(periapsis.TriggerCI, withPaths)
	request.ApprovedSecrets = map[string]bool{}
	workflow, err := Build(testConfig(), request)
	if err != nil {
