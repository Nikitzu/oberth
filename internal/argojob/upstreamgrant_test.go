...
// only restate a constraint that is already enforced.
//
// It matters because the installer seeds the org's forge token at
// oberth/upstream/<org>/github-token. Requiring a grant meant every repository
// that pulls a private package needed an administrator command before its
// first push would do anything but fail at admission.
...
// function: a branch run is bound to the ci-secrets identity, whose policy
// covers the upstream subtree only, so admitting a system declaration would
// produce a run that fails at Vault read time instead of at admission.
