...
func registerRunIssueWork(ctx context.Context, tx *sql.Tx, run model.Run, now int64) error {
	if run.RefKind != model.RefBranch || run.Trigger == "promotion" || run.Trigger == "plan" || run.Trigger == "apply" {
		return nil
	}
...
func runIssueBody(run model.Run, failureTail string) string {
	...
	if burn, step := strings.TrimSpace(run.FailedBurn), strings.TrimSpace(run.FailedStep); burn != "" && step != "" {
		sections = append(sections, fmt.Sprintf("full step log: run_logs %s %s %s", run.ID, burn, step))
	} else if step != "" && sha != "" {
		// Legacy fallback: no burn recorded, fall back to SHA-based hint.
