...
func measureAuditMutationVerifier(t *testing.T, rowCount int) auditMutationVerifierMeasurement {
	...
	verifiedHead, verifiedAnchor, metrics, err := verifyAuditMutationState(ctx, tx, intents, witnesses, true)
	if err == nil {
		err = tx.Commit()
	}
...
func measureAuditMutationVerifierFailClosed(t *testing.T) bool {
	...
	defer func() { _ = tx.Rollback() }()
	_, _, _, err = verifyAuditMutationState(ctx, tx, []model.AuditWitnessIntent{intent}, []model.AuditWitness{witness}, true)
	if err == nil {
