...
// TailAuditAction verifies the complete audit chain and returns its final
// action. It fails on an empty chain. Used by witness-genesis adoption resume
// detection, which must recognize an already-appended acknowledgment.
func (s *Store) TailAuditAction(ctx context.Context) (model.AuditAction, error) {
	var tail model.AuditAction
	head, err := walkVerifiedAuditChain(ctx, s.db, func(action model.AuditAction) {
		tail = action // the visitor runs in ID order; the last call is the tail
	})
	if err != nil {
		return model.AuditAction{}, err
	}
	if head.ID == 0 {
		return model.AuditAction{}, fmt.Errorf("%w: audit chain is empty", ErrInvalidState)
	}
	return tail, nil
}
...
func (s *Store) VerifyAuditStateSince(ctx context.Context, sinceID int64, sinceDigest []byte) (model.AuditHead, error) {
	...
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return model.AuditHead{}, fmt.Errorf("begin incremental audit verification: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	head, err := verifyAuditChainSince(ctx, tx, sinceID, sinceDigest)
	if err != nil {
		return model.AuditHead{}, err
	}
	if err := verifyPendingPublicationIntents(ctx, tx); err != nil {
	...
	if err := tx.Commit(); err != nil {
		return model.AuditHead{}, fmt.Errorf("finish incremental audit verification: %w", err)
	}
	return head, nil
}
...
func (s *Store) VerifyAuditMutationState(
	...
	return s.verifyAuditMutationStateInTx(ctx, intents, witnesses, true, whileVerified)
}

// VerifyAuditMutationStateUnanchored is VerifyAuditMutationState for
// deployments without a timestamp authority: no signed checkpoint is ever
// recorded, so zero persisted anchors is the healthy state. Every anchor that
// does exist (from an earlier anchored configuration) must still reference the
// verified chain and a recovered external witness.
func (s *Store) VerifyAuditMutationStateUnanchored(
	ctx context.Context,
	intents []model.AuditWitnessIntent,
	witnesses []model.AuditWitness,
	whileVerified func(model.AuditHead, model.AuditAnchor) error,
) (model.AuditHead, model.AuditAnchor, error) {
	return s.verifyAuditMutationStateInTx(ctx, intents, witnesses, false, whileVerified)
}

func (s *Store) verifyAuditMutationStateInTx(
	ctx context.Context,
	intents []model.AuditWitnessIntent,
	witnesses []model.AuditWitness,
	requireAnchor bool,
	...
	defer func() { _ = tx.Rollback() }()
	head, anchor, _, err := verifyAuditMutationState(ctx, tx, intents, witnesses, requireAnchor)
	if err != nil {
...
func verifyAuditMutationState(
	...
	anchor, err := verifyAuditMutationAnchors(ctx, observed, snapshot, witnessedHeads, requireAnchor)
	if err != nil {
		return model.AuditHead{}, model.AuditAnchor{}, observed.metrics(), err
	}
...
func verifyAuditChainSince(ctx context.Context, querier auditStateQuerier, sinceID int64, sinceDigest []byte) (model.AuditHead, error) {
	if sinceID <= 0 || len(sinceDigest) != sha256.Size {
		return verifyAuditChain(ctx, querier)
	}
	var storedDigest []byte
	err := querier.QueryRowContext(ctx, `SELECT sha256 FROM audit_actions WHERE id = ?`, sinceID).Scan(&storedDigest)
	if err != nil {
		return model.AuditHead{}, fmt.Errorf("verify audit chain anchor at %d: %w", sinceID, err)
	}
	if !bytes.Equal(storedDigest, sinceDigest) {
		return model.AuditHead{}, fmt.Errorf("audit chain anchor at %d has been tampered: stored digest differs from verified digest", sinceID)
	}
	rows, err := querier.QueryContext(ctx, `
SELECT id, actor, action, resource_type, resource_id, details, previous_sha256, sha256, created_at
FROM audit_actions WHERE id > ? ORDER BY id`, sinceID)
	if err != nil {
		return model.AuditHead{}, fmt.Errorf("read audit chain suffix: %w", err)
	}
	defer func() { _ = rows.Close() }()
	expectedID := sinceID + 1
	previous := append([]byte(nil), sinceDigest...)
	head := model.AuditHead{ID: sinceID, SHA256: append([]byte(nil), sinceDigest...)}
	for rows.Next() {
		var action model.AuditAction
		var createdAt int64
		if err := rows.Scan(&action.ID, &action.Actor, &action.Action, &action.ResourceType, &action.ResourceID,
	...
	if err := rows.Err(); err != nil {
		return model.AuditHead{}, fmt.Errorf("iterate audit chain suffix: %w", err)
	}
	return head, nil
}
