...
var (
	// uplinkTokenLocation is the bearer token the written client config reads.
	uplinkTokenLocation = secretLocation{
		service:  "oberth-token",
		label:    "Oberth uplink token",
		passPath: "oberth/token",
	}
	...
	// print scrolled off and the store was lost twice in one day: an
	// initialized OpenBao whose unseal key nobody has is a permanently sealed
