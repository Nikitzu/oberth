...
//
// The root token and the unseal key go to the same host secret store the
// bearer token already goes to, and what reaches the screen is where they
// were put and how to read them back -- not the values. A credential nobody
// has to copy off a terminal is a credential that does not get lost when the
// terminal scrolls, and an initialized OpenBao whose unseal key is gone is
// permanently sealed, so this is the credential pair where losing it costs
// the whole store.
//
