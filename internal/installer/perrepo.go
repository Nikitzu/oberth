...
// (all orgs), this scopes to the org the repository belongs to and to that
// repository's own namespace: path "oberth/data/upstream/<org>/*" and
...
// credentials.
//
// The org-level rule is a deliberate divergence from upstream, and it exists to
// keep this policy consistent with what admission already admits. This fork
// authorizes upstream-scoped paths structurally (ce51495, PR #27): a path under
// the repository's own org is admitted by scoped.Authorize without an
// approval-table row, and periapsis.UpstreamSecretPath permits the org-scoped
// form oberth/upstream/<org>/<secret> as well as the repo-scoped form. Without
// the org-level rule here, a release run for a repository that holds any grant
// would select this identity, pass admission on an org-scoped declaration, and
// then fail the Vault read with a 403, which is a worse signal than refusing
// the run and is a regression against the shared policy's upstream/* rule.
//
// Cross-org isolation is unaffected: the wildcard stops at the repository's own
// org, so no identity can read another org's subtree.
func PerRepoPolicy(kvPrefix, org, repo string, grantPaths []string) string {
	var builder strings.Builder
	fmt.Fprintf(&builder, `# Per-repo identity: read-only on this repo's org and its own namespace.
# Managed by oberth install --install-secretstore. Do not edit manually.
# The org-level rule matches this fork's structural authorization of
# upstream-scoped paths (ce51495, PR #27); see PerRepoPolicy's doc comment.
...
path "%s/data/upstream/%s/%s/*" {
  capabilities = ["read"]
}`, kvPrefix, org, kvPrefix, org, repo)
