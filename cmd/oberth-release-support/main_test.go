...
func TestFailPeriodicHeartbeatWriterAllowsInitialHeartbeatAfterChildOutput(t *testing.T) {
	t.Parallel()
	writer := &failPeriodicHeartbeatWriter{}
	if _, err := writer.Write([]byte("child-pid=123\n")); err != nil {
		t.Fatal(err)
	}
	heartbeat := []byte("release heartbeat: publish-images active\n")
	if _, err := writer.Write(heartbeat); err != nil {
		t.Fatalf("initial heartbeat raced with child output: %v", err)
	}
	if _, err := writer.Write(heartbeat); !errors.Is(err, io.ErrClosedPipe) {
		t.Fatalf("periodic heartbeat result = %v, want io.ErrClosedPipe", err)
	}
}
...
func (writer *failPeriodicHeartbeatWriter) Write(value []byte) (int, error) {
	...
	if bytes.HasPrefix(value, []byte("release heartbeat:")) {
		writer.heartbeats++
		if writer.heartbeats > 1 && strings.Contains(writer.buffer.String(), "child-pid=") {
