...
func TestServerVersionIsReadFromTheStatusPayload(t *testing.T) {
	t.Parallel()
	if got := serverVersionFrom([]byte(`{"version":"0.18.0","database":"ok"}`)); got != "0.18.0" {
		t.Fatalf("version = %q", got)
	}
	// A payload with no version, and a body that is not JSON at all, are both
	// "not known" rather than an error: status still printed.
	if got := serverVersionFrom([]byte(`{"database":"ok"}`)); got != "" {
		t.Fatalf("version = %q, want empty", got)
	}
	if got := serverVersionFrom([]byte("not json")); got != "" {
		t.Fatalf("version = %q, want empty", got)
	}
}
